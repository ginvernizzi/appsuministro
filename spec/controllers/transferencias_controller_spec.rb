require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe TransferenciasController do

  # This should return the minimal set of attributes required to create a valid
  # Transferencia. As you add validations to Transferencia, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) { { "fecha" => "2015-02-02" } }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # TransferenciasController. Be sure to keep this updated too.
  let(:valid_session) { {} }

  describe "GET index" do
    it "assigns all transferencias as @transferencias" do
      transferencia = Transferencia.create! valid_attributes
      get :index, {}, valid_session
      assigns(:transferencias).should eq([transferencia])
    end
  end

  describe "GET show" do
    it "assigns the requested transferencia as @transferencia" do
      transferencia = Transferencia.create! valid_attributes
      get :show, {:id => transferencia.to_param}, valid_session
      assigns(:transferencia).should eq(transferencia)
    end
  end

  describe "GET new" do
    it "assigns a new transferencia as @transferencia" do
      get :new, {}, valid_session
      assigns(:transferencia).should be_a_new(Transferencia)
    end
  end

  describe "GET edit" do
    it "assigns the requested transferencia as @transferencia" do
      transferencia = Transferencia.create! valid_attributes
      get :edit, {:id => transferencia.to_param}, valid_session
      assigns(:transferencia).should eq(transferencia)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Transferencia" do
        expect {
          post :create, {:transferencia => valid_attributes}, valid_session
        }.to change(Transferencia, :count).by(1)
      end

      it "assigns a newly created transferencia as @transferencia" do
        post :create, {:transferencia => valid_attributes}, valid_session
        assigns(:transferencia).should be_a(Transferencia)
        assigns(:transferencia).should be_persisted
      end

      it "redirects to the created transferencia" do
        post :create, {:transferencia => valid_attributes}, valid_session
        response.should redirect_to(Transferencia.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved transferencia as @transferencia" do
        # Trigger the behavior that occurs when invalid params are submitted
        Transferencia.any_instance.stub(:save).and_return(false)
        post :create, {:transferencia => { "fecha" => "invalid value" }}, valid_session
        assigns(:transferencia).should be_a_new(Transferencia)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Transferencia.any_instance.stub(:save).and_return(false)
        post :create, {:transferencia => { "fecha" => "invalid value" }}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested transferencia" do
        transferencia = Transferencia.create! valid_attributes
        # Assuming there are no other transferencias in the database, this
        # specifies that the Transferencia created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Transferencia.any_instance.should_receive(:update).with({ "fecha" => "2015-02-02" })
        put :update, {:id => transferencia.to_param, :transferencia => { "fecha" => "2015-02-02" }}, valid_session
      end

      it "assigns the requested transferencia as @transferencia" do
        transferencia = Transferencia.create! valid_attributes
        put :update, {:id => transferencia.to_param, :transferencia => valid_attributes}, valid_session
        assigns(:transferencia).should eq(transferencia)
      end

      it "redirects to the transferencia" do
        transferencia = Transferencia.create! valid_attributes
        put :update, {:id => transferencia.to_param, :transferencia => valid_attributes}, valid_session
        response.should redirect_to(transferencia)
      end
    end

    describe "with invalid params" do
      it "assigns the transferencia as @transferencia" do
        transferencia = Transferencia.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Transferencia.any_instance.stub(:save).and_return(false)
        put :update, {:id => transferencia.to_param, :transferencia => { "fecha" => "invalid value" }}, valid_session
        assigns(:transferencia).should eq(transferencia)
      end

      it "re-renders the 'edit' template" do
        transferencia = Transferencia.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Transferencia.any_instance.stub(:save).and_return(false)
        put :update, {:id => transferencia.to_param, :transferencia => { "fecha" => "invalid value" }}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested transferencia" do
      transferencia = Transferencia.create! valid_attributes
      expect {
        delete :destroy, {:id => transferencia.to_param}, valid_session
      }.to change(Transferencia, :count).by(-1)
    end

    it "redirects to the transferencias list" do
      transferencia = Transferencia.create! valid_attributes
      delete :destroy, {:id => transferencia.to_param}, valid_session
      response.should redirect_to(transferencias_url)
    end
  end

end
